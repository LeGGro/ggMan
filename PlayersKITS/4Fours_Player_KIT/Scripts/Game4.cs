...
public class Game4 : MonoBehaviour
{
    ...
    public int queue = 0;
    private Vector2 previewTetrominoPosition = new Vector2(103, 16);
    #endregion
    #region //// Audio Params ////
    ...
    public void UpdateGrid(Tetramino4 tetromino)
    {
        for (int y = 0; y < gridHeight; ++y)
        {
            ...
            {
                Vector2 pos = Rounding( mino.position- new Vector3(90f,0,0));
                if (pos.y < gridHeight)
                {
    ...
    public bool CheckIsAboveGrid(Tetramino4 Tetramino4)
    {
        for (int x = 0; x < gridWidth; ++x)
        {
            foreach (Transform mino in Tetramino4.transform)
            {
        ...
        if (!isGameOver)
        {
            if (!gameIsStarted)
            {
                ...
                nextTetromino = (GameObject)Instantiate(Resources.Load(GetRandomTetromino(rando.mass[queue])),
                    /*parent.transform.position +*/ /*new Vector3(55f, 20f, 0)*/new Vector3(95f, 20f, 0),
                    Quaternion.identity);
                queue++;
                previewNextTetromino = (GameObject)Instantiate(Resources.Load(GetRandomTetromino(rando.mass[queue])),
                    previewTetrominoPosition,
                    Quaternion.identity);
                previewNextTetromino.GetComponent<Tetramino4>().enabled = false;
                queue++;
            }
            else 
            {
                previewNextTetromino.transform.localPosition = new Vector2(95f, 20f);
                nextTetromino = previewNextTetromino;
                nextTetromino.GetComponent<Tetramino4>().enabled = true;
    ...
    {
        string randomTetrName = "T Tetramino4";
        switch (randomTetr)
        {
            case 1:
                randomTetrName = "T Tetramino4";
                break;
            case 2:
                randomTetrName = "J Tetramino4";
                break;
            case 3:
                randomTetrName = "L Tetramino4";
                break;
            case 4:
                randomTetrName = "I Tetramino4";
                break;
            case 5:
                randomTetrName = "O Tetramino4";
                break;
            case 6:
                randomTetrName = "Z Tetramino4";
                break;
            case 7:
                randomTetrName = "S Tetramino4";
                break;
